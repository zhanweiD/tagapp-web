...
class TagCategory extends Component {
  ...
  @action getMenuList = item => {
    ...
    const readKey = {
      key: 'read',
      value: intl
        .get('ide.src.page-scene.scene-detail.tree.main.emo0rsouonu')
        .d('查看详情'),
      onClick: (key, nodeData) => {
        runInAction(() => {
          this.store.currentTreeItemKey = nodeData.id
    ...
    const loop = tree => {
      ...
      tree.forEach(item => {
        if (item.children && item.children.length) {
          ...
          arr.push(
            <DtTreeNode {...dtTreeNodeProps}>{loop(item.children)}</DtTreeNode>
          )
        } else {
          const dtTreeNodeProps = {
            nodeData: item,
        ...
        <DtTreeBox {...treeBoxProps}>
          {(() => {
            ...
            if (!this.store.treeLoading && treeData.length) {
              return <DtTree {...treeProps}>{loop(treeData)}</DtTree>
            }
          })()}
        </DtTreeBox>
        <ModalCategoryEdit />
