...
const createInputPlaceholder = label =>
  label
    ? intl
        .get('ide.src.component.form-component.09m5e9mojnim', { label: label })
        .d('请输入{label}')
...
const createSelectPlaceholder = label =>
  label
    ? intl
        .get('ide.src.component.form-component.129o821xg0n', { label: label })
        .d('请选择{label}')
...
const SelectTypes = ({ label, placeholder, options = [], ...rest }) => {
  ...
  if (
    options.length &&
    options[0].name !==
      intl.get('ide.src.component.comp.search.r6a65smbvr').d('全部') &&
    rest.defaultAll
  ) {
    mergeOption.unshift({
      name: intl.get('ide.src.component.comp.search.r6a65smbvr').d('全部'),
...
export const mergeRules = (rules, label) => {
  ...
  const map = {
    ...
    '@max32': {
      max: 32,
      message: intl
        .get('ide.src.component.form-component.ccct1ih0am5')
        .d('输入不能超过32个字符'),
    },
    '@max128': {
      max: 128,
      message: intl
        .get('ide.src.component.form-component.q56ocecnzan')
        .d('输入不能超过128个字符'),
    },
    '@enNamePattern': {
