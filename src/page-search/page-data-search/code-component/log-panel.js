    ...
    return (
      <Fragment>
        ...
        >
          ...
          >
            <LegacyIcon
              type={store.isMinLog ? 'up-square-o' : 'down-square-o'}
            />
          </Button>
          {store.resultActiveKey === 'running_log' ? (
            <Tooltip
              ...
              title={intl
                .get(
                  'ide.src.page-search.page-data-search.code-component.log-panel.8nd2y3k1hgy'
                )
                .d('点击可以清空已有的日志')}
            >
              <Button
                ...
                title={intl
                  .get(
                    'ide.src.page-search.page-data-search.code-component.log-panel.7gx3kojigpc'
                  )
                  .d('清空日志')}
          ...
          {store.isMinLog ? null : (
            <div
              ...
              onMouseDown={store.onDraggableLogMouseDown}
            />
          )}
            ...
            {store.tableData.map((item, index) => {
              ...
              return (
                <TabPane
                  tab={intl
                    .get(
                      'ide.src.page-search.page-data-search.code-component.log-panel.6vmbeaq07mw',
                      { index: index }
                    )
                    .d('运行结果{index}')}
                ...
                >
                  <ResultTable
                    ...
                    ref={p =>
                      (store[
                        `resultTableHeight${item.taskInstance}${item.resultId}`
                      ] = p)
                    }
                  />
                </TabPane>
