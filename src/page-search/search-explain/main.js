  ...
  return (
    <div className="FBH group-explain">
      <Anchor className="anchor" onClick={onClick}>
        <Link href="#screen-config" title="筛选设置">
          <Link href="#screen-config-contrast" title="对比项" />
          <Link href="#screen-config-action" title="操作符" />
          <Link href="#screen-config-value" title="对比值" />
        </Link>
        <Link href="#out-config" title="输出设置">
          <Link href="#out-config-contrast" title="对比项" />
        </Link>
        <Link href="#case" title="案例">
          ...
          <Link href="#case-search" title="数据查询" />
        </Link>
      </Anchor>
      <div className="anchor-content">
        <div>
          <h2 id="screen-config">筛选设置</h2>
          <h3 id="screen-config-contrast">对比项</h3>
          <p>下面的对比项类型可以在筛选设置中使用：</p>
          <Table columns={[{
            title: '对比项类型',
            dataIndex: 'type',
          }, {
            ...
            pagination={false}
          />
          <h3 id="screen-config-action">操作符</h3>
          <p>下面的操作符可以在筛选设置中使用：</p>
          <Table columns={[{
            title: '操作符',
            dataIndex: 'type',
          }, {
            ...
            pagination={false}
          />
          <h3 id="screen-config-value">对比值</h3>
          <p>面的对比值类型可以在筛选设置中使用：</p>
          <Table columns={[{
            title: '对比值类型',
            dataIndex: 'type',
          }, {
            ...
            dataSource={[{
              ...
              type: '参数',
              desrc: '等于、不等于、大于、大于等于、小于、小于等于、区间、包含、不包含，支持对比值类型为参数。参数需要定义参数名跟默认值'
            }]}
            bordered
            size="small"
            className="pr16 pl16 mb16"
            pagination={false}
          />
          <h2 id="out-config">输出设置</h2>
          <h3 id="out-config-contrast">对比项</h3>
          <p>下面的对比项类型可以在筛选设置中使用：</p>
          <Table columns={[{
            title: '对比值类型',
            dataIndex: 'type',
          ...
          />
          <h2 id="case">案例</h2>
          <p>需求：某电商需要找出最近30天浏览过店铺的女性会员，返回会员号、手机号、姓名、性别、最近三十天浏览次数</p>
          <p>在这里我们有如下对象和标签信息：</p>
          <h3 id="case-object">对象object</h3>
          <ul>
            <li>实体对象：会员（member）、浏览记录（Browse）</li>
            <li>简单关系对象：会员产生浏览记录（member_Browse）</li>
          </ul>
          ...
          <p>分析：在做可视化数据查询的时候，无需关心底层的表，只需关注对象与标签即可</p>
        </div>
      </div>
    </div>
  )
