  ...
  return (
    <div className="FBH group-explain">
      <Anchor className="anchor" onClick={onClick}>
        <Link href="#tql-explain" title="TQL是什么" />
        <Link href="#tql-search" title="TQL查询数据">
          <Link href="#limit" title="限制" />
          <Link href="#function" title="函数" />
          <Link href="#params" title="使用参数" />
        ...
        <Link href="#case" title="案例">
          ...
          <Link href="#tql-example" title="TQL例子" />
        </Link>
      </Anchor>
      <div className="anchor-content markdown-body">
        <div>
          <h2 id="tql-explain">TQL是什么</h2>
          <p>TQL（Tag Query Language）是标签查询语言，是一种基于业务逻辑模型的数据查询语言。标签查询语言与结构化查询语言（SQL）中的数据查询语言（DQL）类似，用以在业务逻辑模型之上从表中获得数据。TQL仅支持SELECT(不区分大小写)开头的语法，不支持 INSERT INTO、DELETE、UPDATE 等，常与WHERE，ORDER BY，GROUP BY等结合起来使用。语法上与mysql类似。
业务逻辑模型即标签体系，通过Object对象、tag标签将库表中的数据重新组织。</p>
          <h2 id="tql-search">TQL查询数据</h2>
          <p>TQL为数据查询而生，用户可以写如下通用SELECT模版语法：</p>
          <pre>
            <code>
              {
                `
          ...
          <ul>
            <li>SELECT语句可以使用多个标签，会返回相应标签数据</li>
            <li>你可以使用 WHERE 语句来包含任何条件</li>
            <li>条件中某个标签为空时候，需要使用关键字IS NULL 来表示</li>
            <li>可以使用GROUP BY结合COUNT函数来分组统计</li>
            <li>可以使用ORDER BY 对结果进行排序</li>
            <li>你可以使用 LIMIT 属性来设定返回的记录数，目前支持最大的记录条数为5000</li>
          </ul>
          <h3 id="limit">限制</h3>
          <pre>
            <code>
          ...
          <ul>
            <li>不支持SELECT * 这种全量标签形式，必须要指定明确的查询列</li>
            <li>标签、对象可以使用别名，但是别名不能是以"$" 开头的字符串</li>
            <li>不支持子查询，类似如下这种</li>
          </ul>
          <pre>
            <code>
          ...
          <Table columns={[{
            ...
            dataSource={[{
              ...
              type: 'COUNT(col)',
              desrc: '返回指定列中非NULL值/行的个数（当函数参数为星号*时不会忽略）'
            }, {
              type: 'MIN(col)',
              desrc: '返回指定列的最小值'
            }, {
              ...
              desrc: '取str左边第start位置起，到end位置字长的字符串'
            }, {
              type: 'DATE_FORMAT(date,fmt)',
              desrc: '依照字符串fmt格式化日期date值'
            }, {
          ...
          <h3 id="order-by">ORDER BY</h3>
           <ul>
            <li>支持使用任何标签来作为排序的条件，从而返回排序后的查询结果</li>
            <li>支持设定多个标签来排序</li>
            <li>支持使用 ASC 或 DESC 关键字来设置查询结果是按升序或降序排列。 默认情况下，它是按升序ASC排列</li>
            <li>被排序的标签必须要出现在SELECT 后面的标签中，否则会查询失败</li>
            <li>样例</li>
          </ul>
          <pre>
            <code>
          ...
          <h3 id="group-by">GROUP BY</h3>
           <ul>
            <li>GROUP BY 语句根据一个或多个列对结果集进行分组。在分组的列上我们可以使用 COUNT, SUM, AVG,等函数。被分组的标签必须要出现在SELECT 后面的标签中，否则会查询失败</li>
            <li>样例</li>
          </ul>
          <pre>
            <code>
          ...
          <h3 id="like">Like</h3>
           <ul>
            <li>精确匹配：</li>
              <ul>
                <li>id：用户传入的标签数据等于id</li>
              </ul>
            <li>前缀匹配：</li>
              <ul>
                <li>id%：标签数据以Id开头，以一至多个字符结尾</li>
              </ul>
            <li>后缀匹配：</li>
              <ul>
                <li>%id：标签数据以一至多个任意字符开头，以Id结尾</li>
              </ul>
            <li>全模糊匹配：</li>
              <ul>
                <li>%id%：标签数据以一至多个任意字符开头，中间匹配Id，以一至多个任意字符结尾</li>
              </ul>
            <li>样例</li>
          </ul>
          <pre>
          ...
          <p>TQL的case when操作Mysql里面一样，支持：</p>
          <ul>
            <li>支持两种方式，简单case 函数，case搜索函数</li>
            <li>样例</li>
          </ul>
          <pre>
            <code>
          ...
          <h2 id="case">案例</h2>
          <p>需求：加工出会员最近三十天购买金额、最近三十天下单次数</p>
          <p>在这里我们有如下对象和标签信息：</p>
          <h3 id="case-object">对象object</h3>
          <ul>
            <li>实体对象：会员（member）、购买记录（buyer）</li>
            <li>简单关系对象：会员产生购买记录（member_buyer）</li>
          </ul>
          <h3 id="case-object-tag">对象下的标签tag</h3>
          <ul>
            <li>会员（member）：会员号（id_number）、手机号（phone）、姓名（name）、性别（sex）</li>
            <li>购买记录（buyer）：购买id（buyer_id）、购买时间（buyer_time）、购买金额（buyer_money）</li>
          ...
          <p>分析：我们在写TQL的时候，无需关心表有几张，只需要从对象和标签角度出发，直接将对象当作表，标签当作字段来处理！，我们在编写TQL界面，自定义一个time参数，值为昨天</p>
          <h3 id="tql-example">TQL例子</h3> 
          <pre>
            <code>
              {
                `
              select
                member.id_number ,
              case 
                when member.sex = '1' then '男'
                when member.sex = '2' then '女'
              else '未知' end sex,
                count（*） as charge,
                sum(buyer.buyer_money) as money
              from
                member
