  ...
  $.addFlex = function (t, p) {
    ...
    p = $.extend({ // apply default properties
      ...
      __mw: { // extendable middleware function holding object
        datacol(p, col, val) { // middleware for formatting data columns
          let _col = (typeof p.datacol[col] === 'function') ? p.datacol[col](val) : val //format column using function
      ...
      getGridClass(g) { // get the grid class, always returns g
        return g
      },
    ...
    var g = {
      hset: {},
      rePosDrag() {
        let cdleft = 0 - this.hDiv.scrollLeft
        ...
        $('thead tr:first th:visible', this.hDiv).each(function () {
          const n = $('thead tr:first th:visible', g.hDiv).index(this)
          let cdpos = parseInt($('div', this).width())
      ...
      fixHeight(newH) {
        ...
        let hdHeight = $(this.hDiv).height()
        $('div', this.cDrag).each(
          function () {
            $(this).height(newH + hdHeight)
          }
        )
        let nd = parseInt($(g.nDiv).height(), 10)
        ...
        let hrH = g.bDiv.offsetTop + newH
        if (p.height != 'auto' && p.resizable) hrH = g.vDiv.offsetTop
        $(g.rDiv).css({
          height: hrH,
        })
      },
      dragStart(dragtype, e, obj) { // default drag function start
        if (dragtype == 'colresize' && p.colResize === true) { // column resize
          $(g.nDiv).hide()
          $(g.nBtn).hide()
          let n = $('div', this.cDrag).index(obj)
          let ow = $(`th:visible div:eq(${n })`, this.hDiv).width()
      ...
      dragMove(e) {
        if (this.colresize) { // column resize
          const {n} = this.colresize
          var diff = e.pageX - this.colresize.startX
          let nleft = this.colresize.ol + diff
          let nw = this.colresize.ow + diff
          if (nw > p.minwidth) {
      ...
      dragEnd() {
        if (this.colresize) {
          const {n} = this.colresize
          let {nw} = this.colresize
          $(`th:visible div:eq(${n })`, this.hDiv).css('width', nw)
          $('tr', this.bDiv).each(
            function () {
              const $tdDiv = $(`td:visible div:eq(${n })`, this)
              $tdDiv.css('width', nw)
          ...
          if ($.cookies) {
            const {name} = p.colModel[n] // Store the widths in the cookies
      ...
      toggleCol(cid, visible) {
        const ncol = $(`th[axis='col${cid }']`, this.hDiv)[0]
        let n = $('thead th', g.hDiv).index(ncol)
        let cb = $(`input[value=${cid }]`, g.nDiv)[0]
        if (visible == null) {
          visible = ncol.hidden
        }
      ...
      switchCol(cdrag, cdrop) { // switch columns
        $('tbody tr', t).each(
          function () {
        ...
        if (p.dataType == 'json') {
          $.each(data.rows, function (i, row) {
            ...
            $('thead tr:first th', g.hDiv).each( // add cell
              function () {
                ...
                let idx = $(this).attr('axis').substr(3)
                td.align = this.align
                ...
                const offs = td.innerHTML.indexOf('<BGCOLOR=')
                if (offs > 0) {
          ...
          $('rows row', data).each(function () {
            ...
            $('thead tr:first th', g.hDiv).each(function () {
              ...
              let text = $(`cell:eq(${  idx  })`, robj).text()
              let offs = text.indexOf('<BGCOLOR=')
              if (offs > 0) {
                $(td).css('background', text.substr(offs + 7, 7))
              }
      ...
      changeSort(th) { // change sortorder
        if (this.loading) {
          return true
        }
        $(g.nDiv).hide()
      ...
      buildpager() { // rebuild pager based on new properties
        $('.pcontrol input', this.pDiv).val(p.page)
        $('.pcontrol span', this.pDiv).html(p.pages)
        let r1 = p.total == 0 ? 0 : (p.page - 1) * p.rp + 1
        let r2 = r1 + p.rp - 1
        if (p.total < r2) {
          r2 = p.total
        }
      ...
      addCellProp() {
        $('tbody tr td', g.bDiv).each(function () {
          ...
          let n = $('td', $(this).parent()).index(this)
          let pth = $(`th:eq(${n })`, g.hDiv).get(0)
          if (pth != null) {
      ...
      getCellDim(obj) { // get cell prop for editable event
        const ht = parseInt($(obj).height(), 10)
        let pht = parseInt($(obj).parent().height(), 10)
        let wt = parseInt(obj.style.width, 10)
        let pwt = parseInt($(obj).parent().width(), 10)
        let top = obj.offsetParent.offsetTop
        let left = obj.offsetParent.offsetLeft
        let pdl = parseInt($(obj).css('paddingLeft'), 10)
        let pdt = parseInt($(obj).css('paddingTop'), 10)
        return {
          ht,
          wt,
          top,
      ...
      combo_flag: true,
      combo_resetIndex(selObj) {
        if (this.combo_flag) {
          selObj.selectedIndex = 0
        }
        this.combo_flag = true
      },
      combo_doSelectAction(selObj) {
        selObj.selectedIndex = 0
        this.combo_flag = false
      },
      // Add title attribute to div if cell contents is truncated
      addTitleToCell(tdDiv) {
        if (p.addTitleToCell) {
          const $span = $('<span />').css('display', 'none')
      ...
      autoResizeColumn(obj) {
        if (!p.dblClickResize) {
          return
        }
        const n = $('div', this.cDrag).index(obj)
          var $th = $(`th:visible div:eq(${  n  })`, this.hDiv)
          var ol = parseInt(obj.style.left, 10)
          var ow = $th.width()
          var nw = 0
          var nl = 0
          var $span = $('<span />')
    ...
    if (p.colModel) { // create model if any
      ...
      for (var i = 0; i < p.colModel.length; i++) {
        ...
        if (cm) { // only use cm if its defined
          if ($.cookies) {
            const cookie_width = `flexiwidths/${cm.name}` // Re-Store the widths in the cookies
    ...
    if (p.combobuttons && $(g.tDiv2)) {
      ...
      $(tSelect).change(() => { g.combo_doSelectAction(tSelect) })
      $(tSelect).click(() => { g.combo_resetIndex(tSelect) })
      ...
      $(this).empty().append(thdiv).removeAttr('width')
        ...
        .hover(function () {
          ...
          if ($(this).attr('abbr') != p.sortname && !g.colCopy && !g.colresize && $(this).attr('abbr')) {
            ...
            const no = (p.sortorder == 'asc') ? 'desc' : 'asc'
            $('div', this).removeClass(`s${p.sortorder}`).addClass(`s${no}`)
          }
          if (g.colCopy) {
            const n = $('th', g.hDiv).index(this)
            if (n == g.dcoln) {
              return false
            }
            if (n < g.dcoln) {
            ...
            const nv = $('th:visible', g.hDiv).index(this)
            const onl = parseInt($(`div:eq(${nv})`, g.cDrag).css('left'), 10)
            const nw = jQuery(g.nBtn).outerWidth()
            const nl = onl - nw + Math.floor(p.cgwidth / 2)
            $(g.nDiv).hide()
            $(g.nBtn).hide()
            $(g.nBtn).css({
              left: nl,
            ...
            const ndw = parseInt($(g.nDiv).width(), 10)
            $(g.nDiv).css({
              top: g.bDiv.offsetTop,
            })
    ...
    if (p.colResize === true) {
      const cdcol = $('thead tr:first th:first', g.hDiv).get(0)
      if (cdcol !== null) {
        ...
        const cdheight = $(g.bDiv).height()
        const hdheight = $(g.hDiv).height()
        $(g.cDrag).css({
          top: `${-hdheight}px`,
        })
    ...
    if (p.usepager) {
      ...
      $(g.bDiv).after(g.pDiv)
      const html = ` <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">${p.pagetext} <input type="text" size="4" value="1" /> ${p.outof} <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>`
      $('div', g.pDiv).html(html)
      $('.pReload', g.pDiv).click(() => {
        g.populate()
      })
      $('.pFirst', g.pDiv).click(() => {
        g.changePage('first')
      ...
      $('.pcontrol input', g.pDiv).keydown(e => {
        if (e.keyCode == 13) { 
          g.changePage('input')
        }
      })
      ...
      if (p.useRp) {
        let opt = ''
        var sel = ''
        for (let nx = 0; nx < p.rpOptions.length; nx++) {
      ...
      if (p.searchitems) {
        ...
        const sitems = p.searchitems
        let sopt = ''; var 
          sel = ''
        for (let s = 0; s < sitems.length; s++) {
        ...
        $('input[name=q]', g.sDiv).keydown(e => {
          if (e.keyCode == 13) {
            g.doSearch()
          }
        })
        $('select[name=qtype]', g.sDiv).keydown(e => {
          if (e.keyCode == 13) {
            g.doSearch()
          }
        })
    ...
    const gh = $(g.bDiv).height()
    const gtop = g.bDiv.offsetTop
    $(g.block).css({
    ...
    if ($('th', g.hDiv).length) {
      ...
      $('th div', g.hDiv).each(function () {
        const kcol = $(`th[axis='col${cn}']`, g.hDiv)[0]
        let chk = 'checked="checked"'
...
$(this).bind('mousedown', () => {
  ...
  $.fn.selectedRows = function (p) { // Returns the selected rows as an array, taken and adapted from http://stackoverflow.com/questions/11868404/flexigrid-get-selected-row-columns-values
    const arReturn = []
    let arRow = []
    const selector = $(`${this.selector} .trSelected`)


    $(selector).each((i, row) => {
      arRow = []
      let idr = $(row).data('id')
      $.each(row.cells, (c, cell) => {
        let col = cell.abbr
