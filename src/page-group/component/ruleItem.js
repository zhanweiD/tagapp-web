...
import { getNamePattern, getNamePatternD } from '../../common/util'
...
const RuleItem = ({
  ...
  const [tagList, changeTagList] = useState(
    ruleType === 'set-rule' ? drawerConfigTagList : configTagList
  )
  ...
  const onSelectFun = e => {
    ...
    if (+ruleIfBoxKey.slice(-1) === 1) {
      const newTagList = relTagList.filter(d =>
        obj.tagTypeList.includes(d.tagType)
      )
      changeRelRenderTag(newTagList)
    } else {
      const newTagList = tagList.filter(d =>
        obj.tagTypeList.includes(d.tagType)
      )
        ...
        <Input.Group compact>
          {ruleType === 'set-rule' && +ruleIfBoxKey.slice(-1) === 1 ? (
            <FormItem
              ...
              name={[key, 'relId']}
              initialValue={
                rest.relId || (otherEntity[0] && otherEntity[0].objId)
              }
              rules={[
                {
                  required: true,
          ...
          rest.page !== 'detail' ? (
            <Tooltip
              title={intl
                .get('ide.src.page-group.component.ruleItem.hq6zitvsyfm')
                .d('更换关系对象会导致已设置的筛选条件被删除!')}
            >
              <FormItem
                label={null}
                name={[key, 'relId']}
                rules={[
                  {
                    required: true,
                    message: intl
                      .get('ide.src.page-group.component.ruleItem.7p5lik2luwk')
                      .d('请选择关系'),
                  },
                ]}
            ...
            rules={[
              {
                required: true,
                message: intl
                  .get('ide.src.page-group.component.ruleItem.fkduhizrm6')
                  .d('请选择函数'),
              },
            ]}
            initialValue={
              rest.leftFunction ||
              (functionRList.length === 7
                ? intl
          ...
          >
            <Select
              ...
              optionFilterProp="children"
              placeholder={intl
                .get('ide.src.page-group.component.ruleItem.ic1bwg1f4vh')
                .d('选择函数')}
              ...
              >
                {relRenderTag.map(d => (
                  <Option value={d.objIdTagId}>
                    <div title={d.objNameTagName} className="omit">
                      {d.objNameTagName}
                    </div>
                  </Option>
                ))}
            ...
            rules={[
              {
                required: true,
                message: intl
                  .get('ide.src.page-group.component.ruleItem.mlbtfcv9cy8')
                  .d('不能为空'),
              },
            ]}
          ...
          {rest.page === 'detail' ? null : (
            <FormItem label={null}>
              <div className="FBH fs14" style={{ color: '#919eab' }}>
                ...
                ruleType === 'config' &&
                relId ? (
                  <a href onClick={open} className="ml8 fs12">
                    {intl
                      .get('ide.src.page-group.component.ruleItem.aluwgl341q')
                      .d('设置筛选')}
                  </a>
                ) : null}

                {rest.level.length === 3 && rest.isEnd ? (
                  <IconTreeAdd
                    size="14"
                    onClick={() => addCombineItem()}
                    className="ml8"
                  />
                ) : null}
          ...
          ruleType === 'config' &&
          relId ? (
            <a href onClick={open} className="ml8 fs12 mt8">
              {intl
                .get('ide.src.page-group.component.ruleItem.javfdbwljp9')
                .d('查看筛选')}
            </a>
          ) : null}
        </Input.Group>
