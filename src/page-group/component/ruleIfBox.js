...
export default class RuleIfBox extends Component {
  constructor(props) {
    ...
    if (document.getElementById(props.id) && props.detail) {
      ...
      document.getElementById(props.id).style.height = `${
        height > 50 ? height : 50
      }px`
    }
  }
  ...
  renderLine = () => {
    ...
    return data.map(d => {
      if (d.source && d.source) {
        ...
        const points = `${+d.source[0]}, ${+d.source[1]} ${+d.source[0]} ${+d
          .source[1] + height}, ${+d.target[0]} ${+d.target[1]}`
      ...
      if (d.type === 2) {
        return (
          <RuleItem
            ...
            changeRelWithRuleConfig={() =>
              changeRelWithRuleConfig(`${ruleIfBoxKey}-${d.flag}`)
            }
          />
        )
      }
  ...
  addCon = (d, i) => {
    ...
    if (len === 0) {
      ...
      const lastIndex = _.findIndex(
        data,
        d => d.flag === `0-${levelTwoArr.length - 1}`
      )
  ...
  addCombineCon = (itemData, i) => {
    ...
    if (this.props.type === 'config') {
      this.props.changeRuleConfig(
        itemKey,
        `${this.props.ruleIfBoxKey}-${updateItemLevel.join('-')}`
      )
    }
  }
  ...
  delCon = (current, i) => {
    ...
    if (current.level.length === 2) {
      bortherNode = data.filter(
        d => d.level.length === 2 && d.level[0] === current.level[0]
      )
    }
    ...
    if (bortherNode.length === 1) {
      ...
      const bortherIndex = _.findIndex(
        data,
        d => d.flag === `${current.flag.slice(0, -1)}0`
      )
      const borderOriginFlag = data[bortherIndex].flag

      this.props.formRef.current.resetFields([
        `${this.props.ruleIfBoxKey}-${borderOriginFlag}`,
      ])

      const fatherIndex = _.findIndex(
        data,
        d => d.flag === current.flag.slice(0, -2)
      )
      const { x, y, level } = data[bortherIndex]

      data[bortherIndex].x = x - 88
      if (data[bortherIndex].flag === '0-0-0') {
        data[bortherIndex].source = [
          data[fatherIndex].source[0],
          data[fatherIndex].source[1] - 64,
        ]
      } else {
      ...
      if (this.props.type === 'config') {
        this.props.changeRuleConfig(
          `${this.props.ruleIfBoxKey}-${borderOriginFlag}`,
          `${this.props.ruleIfBoxKey}-${level.join('-')}`
        )
      }
    }
  ...
  render() {
    ...
    return (
      <div className="rule-if-box" id={id} style={{ height }}>
        {page === 'detail' ? null : (
          <Button onClick={() => this.addCon()} className="rule-add-btn mt4">
            {intl
              .get(
