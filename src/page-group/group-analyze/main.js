  ...
  @action.bound search(values) {
    ...
    store.getRoportion(
      ...
      data => {
        ...
        this.roportion.setOption(roportionOpt(data))
      ...
      <Provider store={store}>
        ...
        >
          <div>
            ...
            <div
              style={{ display: roportion.time ? 'block' : 'none' }}
              className="analyze-roportion box-border"
            >
              {params.groupId ? (
                <div className="analyze-roportion-name">
                  {roportion.groupName}
                </div>
              ) : null}

              <div
                ref={ref => (this.roportionRef = ref)}
                style={{ height: '150px', width: '150px' }}
              />
              <div className="analyze-roportion-text">
                <div className="fs24">{roportion.groupCount}</div>
                <div>{`在全部实体 ${roportion.totalCount} 个中占比${
                  roportion.totalCount
                    ? (
                        (roportion.groupCount / roportion.totalCount) *
                        100
                      ).toFixed(2)
                    : 0
                }%`}</div>
              ...
              <div className="analyze-roportion-time">
                <span>
                  {intl
                    .get('ide.src.page-group.group-analyze.main.6fitux3rze7')
                    .d('计算于：')}
                </span>
            ...
            <div className="group-analyze header-page">
              {groupId ? (
                <div className="analyze-action bgf content-header">
                  <span>
                    {intl
                      .get('ide.src.page-group.group-analyze.main.yluqtm4k00i')
                      .d('群体画像')}
                  </span>
                  <Button
                    type="primary"
                    ...
                    <NoData
                      text={
                        roportion.time
                          ...
                          : intl
                              .get(
                                'ide.src.page-group.group-analyze.main.ih38xpagmu'
                              )
                              .d('请选择目标群体，完成群体分析')
                      }
                      style={{ marginTop: '15%' }}
